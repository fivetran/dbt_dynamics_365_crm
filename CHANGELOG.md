...
## Bug Fix
- Fixes Databricks compatibility issue in the `string_mapping` macro by using `col.is_number()` instead of `not col.is_string()`, as `is_string()` does not work as expected in Databricks.
